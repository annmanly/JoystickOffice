...
local itemTracker = require("ItemTracker") -- Require the RocketTracker module to manage and update the player's coin count
...
function self:Awake()
  ...
  function self:OnTriggerEnter(other : Collider)
    ...
    if client.localPlayer == player then -- Check if the local player is the same as the player associated with the character
      ...
      local triggerTag = self.gameObject.tag
      print("triggerTag local is " .. triggerTag) -- Log if player info is not found
      print("Trigger object's tag:", self.gameObject.tag)
      ...
      if triggerTag == "rocketprefab" then
        print("Adding rocket")

        itemTracker.AddRocket(Amount) -- Add the specified amount of rocket to the player's total
        -- rocketSpawner.DestroyCoin(self.gameObject) -- Destroy the rocket game object that this script is attached to
      end

      if triggerTag == "duckprefab" then
        print("Adding Duck")

        itemTracker.AddDuck(Amount) -- Add the specified amount of rocket to the player's total
        -- rocketSpawner.DestroyCoin(self.gameObject) -- Destroy the rocket game object that this script is attached to
      end
     end
  end
